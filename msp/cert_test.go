...
func TestSanitizeCert(t *testing.T) {
	...
	for {
		k, cert = generateSelfSignedSM2Cert(t, time.Now())

		_, s, err := utils.UnmarshalECDSASignature(cert.Signature)
		assert.NoError(t, err)

		lowS, err := utils.IsLowSSm2(&k.PublicKey, s)
		assert.NoError(t, err)
...
func TestCertExpiration(t *testing.T) {
	...
	_, cert := generateSelfSignedSM2Cert(t, time.Now().Add(24*time.Hour))
	...
	_, cert = generateSelfSignedSM2Cert(t, time.Now())
	msp.opts.Roots = sm2.NewCertPool()
	msp.opts.Roots.AddCert(cert)
	_, err = msp.getUniqueValidationChain(cert, msp.getValidityOptsForCert(cert))
	assert.NoError(t, err)
...
func generateSelfSignedSM2Cert(t *testing.T, now time.Time) (*sm2.PrivateKey, *sm2.Certificate) {
